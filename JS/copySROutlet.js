...
$(function () {
    ...
    $j('#dvMiddleContent').html("<div style='width:70%;height:36px;margin-top: 5px;background-color:#8dc060;text-align:center;color:#fff;font-size:25px;padding-top:10px;margin-top:10px;margin-left:15%;'><span>Copy Outlet to SR</span></div>" +
        ...
        "<tr><td>Getting From SR ID :</td><td><input type='text' id='txtFromSRId' style='border:1px solid #8dc060;width:200px;height:25px;'/></td><td></td></tr>" +
        "<tr><td>Copy To SR ID :</td><td><input type='text' id='txtToSRId' style='border:1px solid #8dc060;width:200px;height:25px;'/></td><td></td></tr>" +

        "<tr><td></td><td style='text-align:right;'> <button type='button' id='btnSave' style='background-color: #8dc060;border-color: #4cae4c;color: #fff;-moz-user-select: none;background-image: none;border: 1px solid transparent;border-radius: 4px;cursor: pointer;display: inline-block;font-size: 14px;font-weight: 400;line-height: 1.42857;margin-bottom: 0;padding: 6px 12px;text-align: center;vertical-align: middle;white-space: nowrap;'>Copy Outlet</button></td></tr></table></div>");
    ...
    $j("#btnSave").click(function () {
        ...
        var txtFromSRId = $j("#txtFromSRId").val();
        var txtToSRId = $j("#txtToSRId").val();
        ...
        if (ddlCountry == "-1") {
            ...
            alert('Enter Getting From SR ID');
            return;
        }
        else if (txtToSRId == "") {
            alert('Enter Copy To SR ID');
            return;
        }

        $j.ajax({
            type: "POST",
            url: "operation.aspx/CopySROutlet",
            data: "{routeID:'" + routeID + "',txtFromSRId:'" + txtFromSRId + "',txtToSRId:'" + txtToSRId + "'}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
            success: function (response) {
